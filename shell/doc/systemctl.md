...
```
10. 检查某个单元（如 cron.service）是否启用 systemctl is-enabled crond.service
```java
# systemctl is-enabled crond.service
enabled
```
11. 检查某个单元或服务是否运行 systemctl status firewalld.service
...
```
12. 列出所有服务（包括启用的和禁用的） systemctl list-unit-files --type=service
```java
...
```
17. 列出所有系统挂载点 systemctl list-unit-files --type=mount
...
rm '/etc/systemd/system/cups.socke
```
                  ...
                  └─auditd.service @4.563s +301ms
                    └─systemd-tmpfiles-setup.service @4.485s +69ms
                      └─rhel-import-state.service @4.342s +142ms
                        └─local-fs.target @4.324s
                          └─boot.mount @4.286s +31ms
                            └─systemd-fsck@dev-disk-by\x2duuid-79f594ad\x2da332\x2d4730\x2dbb5f\x2d85d19
```
